        ...
        private void update(SignPostEventArgs args)
        {
            ...
            if (state.Type != StateType.Hidden)
            {
                if (RoadSignSettings.DistanceLabel != null)
                {
                    ...
                    //Debug.Log("거리는" + args.Distance.ToString("0"));


                }
                // ------------------------------------------------------------------------------------------ 거리 파악하기 위한 부분 추가----------
                //-------------------------------------------------컨텐츠 장소에서 거리가 일정 미터 이상 벌어지면 컨텐츠 불가능 안내 UI 뜨도록
                if (RoadSignSettings.DirectionLabel != null)
                {
                    ...
                    {
                        IsTooFar = true; 
                        // 각 씬에 존재하는 GameManager가 bool값 받아서 가까운 거리임을 안내하는 ui 액티브 되도록 처리
                    }
                    ...
                    {
                        IscloseEnough = true;
                        //RoadSignSettings.DirectionLabel.text = "이미지 타겟을 찾으세요";
                        Debug.Log("토큰이 가까이 있음"); //목적지에 가까운 상태
                    }
