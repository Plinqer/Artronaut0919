...
#define kReplayKitNativeGameObject              "ReplayKit"
#define kSavingToGalleryFinished                "OnReplayKitSaveToGalleryFinished"//Done

#define kVideoPrepareRecordingStarted           "OnReplayKitPrepareRecordingStarted"
#define kVideoPrepareRecordingFinished          "OnReplayKitPrepareRecordingFinished"
#define kVideoPrepareRecordingFailed            "OnReplayKitPrepareRecordingFailed"
...
- (void) prepareRecording
{
    if(_recordingState == None)
    {
        ...
        NSLog(@"Prepare for recording finished with status %d", success);
            if(success)
                [_listener onReplayKitPrepareRecordingFinished:success];
        }];
    }
}
...
- (void) prepareRecordingInternal :(void(^)(BOOL))callback;
{
    ...
    {
        [_listener onReplayKitPrepareRecordingStarted: TRUE];
        ...
        if(_microphoneEnabled)
        {
            ...
            [audioSession requestRecordPermission:^(BOOL granted) {
                if(!granted)
                {
                    ...
                    [_listener onReplayKitPrepareRecordingFailed: "MICROPHONE_PERMISSION_UNAVAILABLE"];
        ...
        [recorder startCaptureWithHandler:captureHandler completionHandler:^(NSError * _Nullable error)
         {
             ...
             else
             {
                 ...
                 [_listener onReplayKitPrepareRecordingFailed: errorInfo];
...
-(void) onReplayKitPrepareRecordingStarted: (BOOL) status {
    UnitySendMessage(kReplayKitNativeGameObject, kVideoPrepareRecordingStarted, status ? "true" : "false");
}
-(void) onReplayKitPrepareRecordingFinished: (BOOL) status {
    UnitySendMessage(kReplayKitNativeGameObject, kVideoPrepareRecordingFinished, status ? "true" : "false");
}

-(void) onReplayKitPrepareRecordingFailed:(const char*) error {
    UnitySendMessage(kReplayKitNativeGameObject, kVideoPrepareRecordingFailed, error);
}
