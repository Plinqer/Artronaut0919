...
namespace VoxelBusters.ReplayKit.Editor.Build.Xcode
{
    public class XcodeBuildProcessor : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        {
            var     settings        = ReplayKitSettingsEditorUtility.DefaultSettings;
            ...
            if (target != null)
            {
                target.IsEnabled    = settings.IsEnabled;
            }
        }
        ...
        {
            var     settings        = ReplayKitSettingsEditorUtility.DefaultSettings;
            if ((report.summary.platform != BuildTarget.iOS) || (settings == null) || !settings.IsEnabled) return;
        ...
        {
            DebugLogger.Log("[ReplayKit] Updating plist configuration.");
